...
int rknn_lite::interf()
{
    ...
    // std::cout << "inputs[0].buf is ok" << std::endl;
    // 设置rknn的输入数据
    ...
    // std::cout << "get output is ok" << std::endl;
    // // 假设输出结果是图像数据
    // int output_width = width;   // 模型输出图像的宽度
    // int output_height = height; // 模型输出图像的高度
    // int channels = 3;           // 假设输出是RGB图像

    // // 将int8输出转换为OpenCV格式
    // cv::Mat output_image(output_height, output_width, CV_8UC3);

    // // 如果输出是int8类型，需要进行数据转换
    ...
    //         {
    //             // 假设outputs[0].buf是int8类型，转换为uint8
    //             output_image.at<cv::Vec3b>(h, w)[c] = (uint8_t)((int8_t *)outputs[0].buf)[h * output_width * channels + w * channels + c];
    //         }
    //     }
    // }
    ...
    // // 注意：上述处理假设了输出是RGB格式且在outputs[0].buf中，如果格式不同，需要根据具体需求调整。
    ...
    // const float nms_threshold = 0;
    // const float box_conf_threshold = 0;
