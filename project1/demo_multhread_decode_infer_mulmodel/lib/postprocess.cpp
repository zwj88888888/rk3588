...
int loadLabelName(const char *locationFilename, char *label[], int class_num)
{
    ...
    readLines(locationFilename, label, class_num);
    return 0;
}
 ...
 * @param stride 步长
 * @param boxes 目标框坐标向量
 * @param objProbs 目标概率向量
 * @param classId 目标类别向量
 * @param threshold 阈值
 * @param zp 零点偏移
 * @param scale 比例因子
 *
                 ...
                 std::vector<float> &qnt_scales, detect_result_group_t *group, int class_num)
{
    ...
    {
        int ret = 0;
        ret = loadLabelName(LABEL_NALE_TXT_PATH, labels, class_num);
        if (ret < 0)
        {
            return -1;
        }
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        // 获取当前有效目标框的索引
        int n = indexArray[i];
        // 从过滤后的边界框数组中获取当前目标框的坐标信息
        float x1 = filterBoxes[n * 4 + 0];      // 左上角 x 坐标
        float y1 = filterBoxes[n * 4 + 1];      // 左上角 y 坐标
        float x2 = x1 + filterBoxes[n * 4 + 2]; // 右下角 x 坐标（基于宽度）
        float y2 = y1 + filterBoxes[n * 4 + 3]; // 右下角 y 坐标（基于高度）

        // 获取当前目标的类别 ID 和置信度
        int id = classId[n];
        float obj_conf = objProbs[i];

        // 将检测结果的坐标转换为原始图像的坐标并存储在结果数组中
        group->results[last_count].box.left = (int)(clamp(x1, 0, model_in_w) / scale_w);   // 限制左边界值并进行比例缩放
        group->results[last_count].box.top = (int)(clamp(y1, 0, model_in_h) / scale_h);    // 限制上边界值并进行比例缩放
        group->results[last_count].box.right = (int)(clamp(x2, 0, model_in_w) / scale_w);  // 限制右边界值并进行比例缩放
        group->results[last_count].box.bottom = (int)(clamp(y2, 0, model_in_h) / scale_h); // 限制下边界值并进行比例缩放
